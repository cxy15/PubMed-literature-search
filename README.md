...
- 勿将包含真实密钥的 `.env` 提交到公共仓库或随压缩包随意分发。

## 打包分享

向他人分发源码时，请勿包含 **`.env`**（密钥）或 **`.venv`**（体积大且与机器相关）。可在项目根目录执行：

```bash
python3 scripts/package_release.py
```

会在当前目录生成 **`文献检索-分享包.zip`**，内含顶层文件夹 `文献检索/`，已排除 `.venv`、`__pycache__`、`.env`、`.git` 与 `logs/*.log`。

接收方解压后执行 `cp .env.example .env` 并填写配置，再按上文安装依赖即可。
